...
// ---------------------------------------------------------------------------
// branchesFromRawJson — parse a raw backend reply, then derive branches.
//
// NOTE: `RadicleImpl::localListBranches` no longer calls this. It was written
// when the local path derived branches from `getRepo`'s `refs.refs`, the same
// way the seed path does; that turned out to report exactly one branch per
// repository, because `refs.refs` holds the canonical delegate-consensus ref
// while real branches live under `refs/namespaces/<nid>/`. The local path now
// has its own backend entry point.
//
// These tests are kept, and the function with them, because the property they
// pin is not local-specific: "a reply that is not even JSON must report a
// named error rather than an empty branch list" is how any caller of
// `branchesFrom` should behave, and the malformed branch is unreachable
// through a real backend (the Rust side is built with serde_json and cannot
// emit invalid JSON). A hand-crafted string is the only way to exercise it.
// ---------------------------------------------------------------------------
