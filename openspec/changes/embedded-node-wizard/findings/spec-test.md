...
- [ ] **`tester`** — `tst_setup_wizard.qml:506` (`test_a_refused_mode_write_does_not_move_the_mode_in_force`) — the requirement's positive path through `chooseMode` is never exercised
...
- [ ] **`spec-writer`** — spec.md, Requirement "The preflight reports four findings before offering a choice" — the ordering half ("before offering any choice that depends on it") is not testable against these fakes
